export const chants = [
    {
        title: 'Standar MiX',
        lyrics: `Taiga! Faiya! Saiba!\nFaiba! Daiba! Baiba! Jyajya!`
    },
    {
        title: 'Reverse Standar MiX',
        lyrics: `Jya jya!
                Baiba!
                Daiba!
                Faiba!
                Saiba!
                Faiya!
                Taiga!`
    },
    {
        title: 'Urya Oi',
        lyrics: `(Urya!) Oi! 4xi
                Ah, mada ikanai
                (Urya!) Oi! 4x
                Ah, mada ikanai
                Itsu iku no? Ima desho! / Hai se no!`
    },
    {
        title: 'Yes Tiger',
        lyrics: `Iet, iet, iettaiga! /
                Iettaiga, iettaiga, iettaiga
                Faibo Waipa!`
    },
    {
        title: 'Myohontusuke Start',
        lyrics: `Myohontusuke!
                Kasentobijoukyo!
                Jya jya!
                Faibo!
                Waipa!`
    },
    {
        title: 'Myohontusuke Hajimaru',
        lyrics: `Hajimaru yo!
                Hajimaru yo!
                Myohontusuke, hajimaru yo!
                Myohontusuke!
                Kasentobijoukyo!
                Jya jya!
                Faibo!
                Waipa!`
    },
    {
        title: 'Japanese MiX',
        lyrics: `Ah, Mou iccho ikuzo!
                Tora!
                Hi!
                Jinzou!
                Seni!
                Ama!
                Shindou!
                Kasen!
                [Tobi! Joukyo!]`
    },
    {
        title: 'Japanese Hasei MiX',
        lyrics: `Tora! 12x
                Tora hi!
                Jinzou!
                Seni!
                Ama!
                Shindou!
                Kasen!
                Tobi!
                Joukyo!
                [Chou umi u mai nōjō!]`
    },
    {
        title: 'Reverse Japanese MiX',
        lyrics: `Kasen!
                Shindou!
                Ama!
                Seni!
                Jinzou!
                Hi!
                Tora!`
    },
    {
        title: 'Ainu MiX ',
        lyrics: `Ah, sha ikuzo!
                Chape!
                Ape!
                Kara!
                Kira!
                Rara!
                Tusuke!
                Wisupe!
                Kesui! Suisupa!`
    },
    {
        title: 'Ainu MiX Stop Myohontusuke',
        lyrics: `Chape!
                Ape!
                Kara!
                Kira!
                Rara!
                Tusuke!
                Myohontusuke!`
    },
    {
        title: 'Enchou MiX',
        lyrics: `Faibo!
                Waipa!
                Faama!
                Jyasupa!
                Howaipa!
                Kuupa!
                Iesukuraipa!`
    },
    {
        title: 'Spanish MiX',
        lyrics: `Vamos (Bamous)!
                Tigre (Tigure)!
                Fuego!
                Ciber (Shiberu)!
                Fibra (Fibura)!
                Buzo!
                Viber (Biburaru)!
                Jersey (Herusei)!
                Volare (Borare)!
                Limpiaparabrisas (Rimpiaparaburisasu)!`
    },
    {
        title: 'Germany MiX',
        lyrics: `Gēenviā!
                Tīgā!
                Foieru!
                Kūnsu!
                Fībā!
                Tautsā!
                Shubingungu!
                Fēmīfāzā!
                [Firīgen kāruhaintsu shunaidā]/[Ie mange etanaru]`
    },
    {
        title: 'Bisumaruku Ro-Jou MiX',
        lyrics: `Takamaru yo! 2x
                Takamaru hikumaru bisumaruku!
                Shinjimaru! Arushindo! Kazudansu!
                Knee high! Oh hai! *clap*
                Kan chuhai! Oolonghai! Nachuraru high!
                I can fly! *jump*`
    },
    {
        title: 'Bisumaruku Kojou MiX',
        lyrics: `Kan ！ Zen ！ Kiyoshi ！ Iki ！ Zettai ryōiki nīhaiōhai
                Kan chūhai ū ron hai nachuraru hai appurupai
                Sankaku choko pai enzeru pai taopaipai fāru furai
                Picchā furai kyacchā furai sentā furai I can fly!`
    },
    {
        title: 'Pan MiX ',
        lyrics: `Pan pan pan pan pokemon pan!
                Furesshu bureddo ito pan!
                Matsutatako! Matsutatako!
                Yamazaki haru no pan matsuri!`
    },
    {
        title: 'Chikipa MiX',
        lyrics: `Raion! Tora! Chītā!
                Ratarata! Ratorātā!
                Sai! Gorira! Zō!
                Sagōzo! Sagōzo!
                Eiyoo! Eiyoo! Eiyoo! Eiyoo!
                Kapusaishintte Shiteru?
                [Eiyo katede shibou kan/kimo!]`
    },
    {
        title: 'Konton MiX',
        lyrics: `Wa! 6x
                Warudokaosu!
                Shogyou!
                Kogure!
                Shigure!
                Kagura!
                Kongousan shoushusha!
                [Kokuun! Mujou! Sekai konton!]`
    },
    {
        title: 'Reverse Konton MiX',
        lyrics: `Shou! 6x
                Shoushusha!
                Kongousan!
                Kagura!
                Shigure!
                Kogure!
                Shogyou!
                Warudokaosu!`
    },
    {
        title: 'Hogwarts MiX',
        lyrics: `Marufoi! 4x
                Aaaaaa~ *clap* gryffindor!
                Akushio redakuto rumos!
                Uingga-diamu rebiosa!
                Anata no wa, rebiosa!
                Ekusupekuto patoronamu!
                Tomu ridoru! tomu ridoru!
                Borudemottotte shiteru?
                [Hāmaionī! Gurenjā!]`
    },
    {
        title: 'Tokushu Kaihen MiX',
        lyrics: `Uisupekesui!
                Chape chape chape!
                Chape ape kara kina kara ape chape!
                Jokyo tobi jokyo!
                Tora tora tora!
                Tora hi jinzou senni ama shindou kasen!`
    },
    {
        title: 'Kaihen 3 Ren MiX',
        lyrics: `Jinzou faiya faibo waipa!
                Taiga taiga tatatata taiga!
                Chape ape kara kina 2x
                Myohontusuke *clap*, waipa!
                Faiya faiya tora tora kara kina
                Chape ape faama, ama ama jasupa!
                Tora taiga! tora taiga!
                Jinzou senni iettaiga!`
    },
    {
        title: 'Kaihen 3 Ren MiX Tsubasa Vers',
        lyrics: `Kita kita kita kita waitekita!
                Taiga taiga taiga shotto!
                Tora tai tora tai tora taiga!
                Jinzou seni iettaiga!
                Jya jya! Jya jya! Baribaribo!
                Suto suto suto suto sutoraika!
                Karu hainsu shunaida!
                Sukai! Rabu! Hariken!`
    },
    {
        title: 'Yosuiro MiX',
        lyrics: `Yosuiro!
                Konbain!
                Tanbo no yousu ga! *clap*, okashi!`
    },
    {
        title: 'Tsunagaritai MiX',
        lyrics: `Tsunagaritai! Tsunagaritai!
                Yappari ◯◯ to tsunagaritai!
                Tsunagaritai! Tsunagaritai!
                Tsunagaritai kara tsunagaritaiga~
                Faiya~ saiba~ faiba~ daiba~ baiba~
                Jya~jya~ Faibo!
                Injo! jasupa! birubaru! kasen!
                Ieenaruda faboriasuta!`
    },
    {
        title: 'Gachikoi Koujo',
        lyrics: `Iitai koto ga, arun da yo!
                Yappari ◯◯ wa, kawaii yo!
                Suki suki daisuki, yappa suki!
                Yatto mitsuketa, ohimesama!
                Ore ga umarete, kita riyuu!
                Sore wa ◯◯ ni, deau tame!
                Ore to issho ni, jinsei ayumou!
                Sekai de ichiban, aishiteru!
                A, i, shi, te, ru!
                [Zettai kekkon shiyou na!]`
    },
]